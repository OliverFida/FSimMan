...
namespace OF.FSimMan.Database.Migrations.ModPacksFs25Db
{
    ...
    public partial class MigrationToGuid : Migration
    {
        ...
        FOREIGN KEY (ModPackId) REFERENCES ModPacks_New (Id) ON DELETE CASCADE
            ...
            // 3. Helper for GUID generation (Fix: changed '89ab' to '89AB')
            ...
            // A. ModPacks
            migrationBuilder.Sql("CREATE TABLE _Map_ModPack (OldId INTEGER, NewId TEXT);");
            migrationBuilder.Sql($"INSERT INTO _Map_ModPack SELECT Id, ({guidSql}) FROM ModPacks;");
        ...
        FROM ModPacks p JOIN _Map_ModPack m ON p.Id = m.OldId;");

            // B. Mods (Mapping the FK ModPackId via the mapping table)
        ...
        FROM Mods s
        JOIN _Map_ModPack m ON s.ModPackId = m.OldId;");

            // 5. Swap Tables
            migrationBuilder.Sql("DROP TABLE Mods;");
            migrationBuilder.Sql("DROP TABLE ModPacks;");
            migrationBuilder.Sql("DROP TABLE _Map_ModPack;");
