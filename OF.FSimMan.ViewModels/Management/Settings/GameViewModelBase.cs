        ...
        private void HandleAppSettingsModPackAutogenerationNowPossible(object? sender, AppSettingsModPackAutogenerationNowPossibleEventArgs e)
        {
            ExecuteBusy(() =>
            {
                try
                {
                    ...
                    if (modFileInfos.Length == 0) return;

                    if (UiFunctions.ShowQuestion("Would you like to auto-generate a modpack from your default mods folder?")) AutogenerateModPack(modFileInfos);

                    AppSettings.GetGameSettings(_game).IsAutogeneratedModPackExecuted = true;
                }
                catch (Exception ex)
                {
                    UiFunctions.ShowError(ex);
